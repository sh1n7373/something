...
_SESSION_ERRORS = (AuthKeyUnregisteredError, UserDeactivatedBanError, SessionRevokedError)
...
class SenderWorker(QObject):
        ...
        for rec in self.recipients:
            ...
            for i, paste in enumerate(self.pastes):
                ...
                try:
                    ...
                    reply, buttons, lifted = await self._query_spambot(client)
                    self.spamblock_signal.emit(self.worker_id, tag, reply, buttons)
                    if self._stop:
                        await client.disconnect()
                        return
                    if not lifted:
                        self._log("Ограничения не сняты, останавливаемся", "err")
                        await client.disconnect()
                        return
                    tag_completed = False
                    break
                    ...
                    if "database is locked" in err_str:
                        ...
                        while reconnect_try < 15:
                            ...
                            self._log("Не удалось переподключиться, остановка", "err")
                            await client.disconnect()
                            return
    ...
    async def _query_spambot(self, client):
        last_reply, buttons, lifted = "", [], False
        def _lifted(text):
            t = text.lower()
            if len(t) < 20:
                return False
            bad = ("limited", "limitation", "limitazione", "limitado", "limitación",
                   "begrenzt", "einschränkung", "beperkt", "ограничен", "limité",
                   "limitert", "snížen", "omezení", "korlátozás", "sınırlı",
                   "限制", "制限", "제한")
        ...
        try:
            for attempt in range(3):
                ...
                if reply_text:
                    ...
                    if _lifted(reply_text):
                        self._log("Ограничения сняты", "ok")
                        lifted = True
                        break
                    if attempt < 2:
        ...
        try:
            ...
            if current:
                self.result_signal.emit({
                    "device":   current.device_model,
                    "platform": current.platform,
                    "system":   current.system_version,
                    "app":      f"{current.app_name} {current.app_version}",
                    "country":  current.country,
                    "ip":       current.ip,
                    "official": current.official_app,
                })
            else:
                self.error_signal.emit("Не удалось получить данные сессии")
        except Exception as ex:
            self.error_signal.emit(str(ex))
    ...
    async def _load(self):
        ...
        try:
            ...
            async for dialog in client.iter_dialogs(limit=500):
                ...
                tag     = f"@{uname}" if uname else str(entity.id)
                last    = ""
                if dialog.message and getattr(dialog.message, "message", None):
                    last = dialog.message.message[:80]
                result.append({
                    "name":   dialog.name or tag,
                    "tag":    tag,
