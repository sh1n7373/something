...
class MainWindow(QMainWindow):
    ...
    def _toggle_worker_pause(self, idx):
        ...
        all_paused = all(self._worker_paused.get(i, False) for i in self._workers if self._workers.get(i))
        any_paused = any(self._worker_paused.get(i, False) for i in self._workers if self._workers.get(i))
        if all_paused:
            self._paused = True
    ...
    def _toggle_pause(self):
        if self._paused:
            ...
            self._paused = False
            self._pause_btn.setText("Пауза")
            for i, cd in enumerate(self._worker_cards):
                if self._workers.get(i):
                    cd["pause_btn"].setText("Пауза")
                    self._worker_paused[i] = False
            ...
            for i, cd in enumerate(self._worker_cards):
                if self._workers.get(i):
                    cd["pause_btn"].setText("Продолжить")
                    self._worker_paused[i] = True
